...
class ResPPOTrainingState(PyTreeNode):
...
class ResPPO:
    def __init__(
        ...
        def rollout_fn(
            ...
            def play_step_fn(
                ...
                # baseline_value = baseline_network.apply(baseline_params, obs)
    ...
    def init(
        ...
        # critic_params = self._critic_network.init(subkey, fake_obs)
        ...
        training_state = ResPPOTrainingState(
    ...
    def train(
        ...
        training_state: ResPPOTrainingState,
        key: RNGKey,
    ) -> Tuple[ResPPOTrainingState, EnvState]:
        """
        ...
        baseline_params = training_state.baseline_params
        current_std = training_state.current_std
        preference = training_state.preference # shape of (d,)
        ...
        # + self._baseline_network.apply(baseline_params, last_obs)
        ...
        td_lambda_returns, weights = self.calculate_td_lambda_returns(
            final_v_value,
            v_values, 
            jnp.sum(transitions.rewards * preference, axis=-1, keepdims=True), # rollout x batch x 1
