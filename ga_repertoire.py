...
from custom_types import Fitness, Genotype, RNGKey, PyTreeNode



class Repertoire(PyTreeNode, ABC):
...
class GARepertoire(Repertoire):
    ...
    def sample(self, random_key: RNGKey, num_samples: int) -> Tuple[Genotype, RNGKey]:
        ...
        mask = jnp.any(self.fitnesses != -jnp.inf, axis=-1)
        
        # uniform_p = jnp.ones(self.size) / self.size
        p = mask / jnp.sum(mask)
